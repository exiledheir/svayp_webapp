...
/**
 * Вкладка «Алмазы» внутри BillingSheet. Показывает баланс, готовые пакеты со скидкой 200+,
 * свободную сумму с живым итогом и кнопку покупки.
 *
 * Оболочку (фон, жест закрытия, крестик, скролл) держит BillingSheet — здесь только
 * содержимое вкладки.
 *
 * Два режима оплаты:
...
export default function CoinsPanel({
  ...
  needMore?: boolean;
  dark: boolean;
  /** Заголовок нужен, только когда вкладок нет: иначе название стоит в переключателе. */
  showTitle?: boolean;
  ...
  const visibleProviders = visiblePaymentProviders(paymentOptions);
  ...
  return (
    <>
      {/* Header + balance. Название вкладки уже стоит в переключателе сверху —
          второй раз писать «Алмазы» незачем, здесь важен только баланс. */}
      <div className="flex flex-col items-center text-center pt-2 pb-1">
        <Diamond size={40} glow />
        {showTitle && (
          <h2 className="text-[20px] font-extrabold mt-2" style={{ color: ink }}>{t.cn_title}</h2>
        )}
        <div className={`flex items-center gap-1.5 ${showTitle ? 'mt-1' : 'mt-2'}`}>
          <Diamond size={15} />
          <span className="text-[16px] font-extrabold" style={{ color: ink }}>{t.cn_have.replace('{n}', fmt(balance))}</span>
        </div>
